...
st.set_page_config(page_title="Hello", page_icon="👋", initial_sidebar_state="collapsed")
...
name = st.selectbox(
    "Chose the Mega-Rare BIS item to view trading info for",
...
historical = st.checkbox(
    "Include historical values in the graph? (Performance decrease when start date is further from the present)",
    value=False,
)
if historical:
    view_delta = "6h"
...
if len(name) != 0:
    ...
    slider = cols1.slider(
        "Select the start date of the graph",
        max_value=start_date.to_pydatetime(),
        value=end_date.to_pydatetime(),
        min_value=end_date.to_pydatetime(),
        format=format,
    )
    ...
    MODEL = st.selectbox(
        "Chose the Model to predict with",
        [None, "Conv1D", "Linear", "Multi_Step_Dense"],
        label_visibility="visible",
    )
    # Note:
    ...
    if MODEL is not None:
        ...
        # Predicted Data
        ChartHelper.get_altair_chart(
            data.loc[pd.to_datetime(slider) :].reset_index(), name, te.numpy()[0][0]
        )
